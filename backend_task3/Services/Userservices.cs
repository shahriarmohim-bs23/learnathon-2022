...
namespace backend_task3.Services
{
    public class Userservices
    {
        ...
        public async Task UpdateToken(Refreshtoken token,string id)
        {
            FilterDefinition<User> filter = Builders<User>.Filter.Eq("_id", ObjectId.Parse(id));
            UpdateDefinition<User> update = Builders<User>.Update.Set("Refreshtoken", token.Token)
                                                                  .Set("RefreshTokenExpires", token.Expires);
            await _register.UpdateOneAsync(filter, update);
            return;
